...
public class HttpProxyDataPlaneExtension implements ServiceExtension {
    ...
    public void initialize(ServiceExtensionContext context) {
        var portMapping = new PortMapping(API_CONTEXT, 9999, "/proxy");
        portMappingRegistry.register(portMapping);

        var proxyUrl = "http://localhost:%d%s".formatted(
            portMapping.port(),
            portMapping.path()
        );
        generatorService.addGeneratorFunction("HttpData", address ->
            Endpoint.url(proxyUrl)
        );

        generatorService.addResponseGeneratorFunction("HttpData", () ->
            Endpoint.url(proxyUrl)
        );

        webService.registerResource(
            API_CONTEXT,
            new Controller(authorizationService)
        );
    }
    ...
    public static class Controller {
        ...
        public Response get(@Context ContainerRequestContext requestContext) {
            var token = requestContext.getHeaderString(
            ...
            var sourceDataAddress = authorizationService.authorize(
                token,
                emptyMap()
            );
        ...
        public Response postResponse(
            ...
            var dataAddressUrl = sourceDataAddress
                .getContent()
                .getStringProperty(EDC_NAMESPACE + "baseUrl");

            if (
                dataAddressUrl != null &&
                !dataAddressUrl.equals("http://any/response/channel")
            ) {
